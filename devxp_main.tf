...
resource "aws_instance" "Instance-mtdw" {
      ...
      iam_instance_profile = aws_iam_instance_profile.Instance-mtdw_iam_role_instance_profile.name
      key_name = "Instance-mtdw_keyPair"
}

resource "aws_eip" "Instance-mtdw_eip" {
      vpc = true
      instance = aws_instance.Instance-mtdw.id
}

resource "tls_private_key" "Instance-mtdw_keyPair_tls_key" {
      algorithm = "RSA"
      rsa_bits = 4096
}

resource "aws_key_pair" "Instance-mtdw_keyPair" {
      public_key = tls_private_key.Instance-mtdw_keyPair_tls_key.public_key_openssh
      key_name = "Instance-mtdw_keyPair"
}

resource "local_sensitive_file" "Instance-mtdw_keyPair_pem_file" {
      filename = pathexpand("~/.ssh/Instance-mtdw_keyPair.pem")
      file_permission = "600"
      directory_permission = "700"
      content = tls_private_key.Instance-mtdw_keyPair_tls_key.private_key_pem
}
...
resource "aws_iam_instance_profile" "Instance-mtdw_iam_role_instance_profile" {
      name = "Instance-mtdw_iam_role_instance_profile"
      role = aws_iam_role.Instance-mtdw_iam_role.name
}

resource "aws_iam_role" "Instance-mtdw_iam_role" {
      name = "Instance-mtdw_iam_role"
...
output "Instance-mtdw_eip-public-ip" {
    value = aws_eip.Instance-mtdw_eip.public_ip
    sensitive = false
}

output "Instance-mtdw_keyPair-private_key" {
    value = tls_private_key.Instance-mtdw_keyPair_tls_key.private_key_pem
    sensitive = true
}

output "Instance-mtdw-ssh_instructions" {
    value = "To access Instance-mtdw, use: ssh -i ~/.ssh/Instance-mtdw_keyPair.pem ubuntu@<OUTPUTTED_IP)>"
    sensitive = false
}
